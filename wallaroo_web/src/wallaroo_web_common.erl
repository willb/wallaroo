...
-export([generic_entity_exists/3, get_starting_commit/1, generic_find/5, generic_find/6, dump_json/3, generic_to_json/4, generic_from_json/5]).
...
dump_json(Entities, ReqData, Ctx) when is_list(Entities) ->
    ...
    {mochijson:encode({array, [fix_json(Entity) || Entity <- Entities]}), ReqData, Ctx};
dump_json({_Kind, _Dict}=Entity, ReqData, Ctx) ->
    error_logger:warning_msg("Entity is ~p~n", [Entity]),
    {mochijson:encode(fix_json(Entity)), ReqData, Ctx}.
...
generic_from_json(ReqData, Ctx, NewFunc, PutKind, PathPart) ->
    ...
    Data = [{list_to_atom(K), V} || {K, V} <- (mochijson:decoder([{object_hook, fun peel/1}]))(wrq:req_body(ReqData))],
    case wrq:path_info(name, ReqData) of
        undefined ->
		...
		error ->
		    {false, ReqData, Ctx};
		{ok, _} ->
		    from_json_helper(Data, ReqData, Ctx, NewFunc, PutKind, PathPart)
	    end;
        Name ->
	    NamedData = orddict:store(name, Name, Data),
	    from_json_helper(NamedData, ReqData, Ctx, NewFunc, PutKind, PathPart)
    end.
...
from_json_helper(Data, ReqData, Ctx, NewFunc, PutKind, PathPart) ->
    ...
    NewCommit = case Commit of
		    none ->
			wallaroo:put_entity(Name, PutKind, {Kind, Dict});
		    Whence ->
			wallaroo:put_entity(Name, PutKind, {Kind, Dict}, Whence)
		end,
