...
module Wallaroo
   module Version
     def self.init
       ...
       items = Hash[*open("./apps/wallaroo/src/version.hrl", "r") {|f| f.read}.split("\n").grep(/.*?(major|minor|patch|build),\s*(.*?)\}.*/).map {|s| s.gsub(/.*?(major|minor|patch|build),\s*(.*?)\}.*/, "\\1=\\2").chomp}.map {|s| res = s.split("="); [res[0].upcase.to_sym, res[1]]}.flatten]  
