...
module Mrg
  module Grid
    module Config
      module Shell
        class ForceRestart < ::Mrg::Grid::Config::Shell::Command
          ...
          def init_option_parser
            ...
            OptionParser.new do |opts|
              ...
              opts.on("-n", "--nodes NLIST", "the nodes to restart") do |list|
                @nodes = list
              end

              opts.on("-g", "--groups GLIST", "the groups of nodes to restart") do |list|
                @groups = list
              end

              opts.on("-a", "--all", "restart all nodes") do
                @all_nodes = true
              end
            end
          end
          ...
          def act
            ...
            val = Time.now.to_f
            if @all_nodes == true
              store.getDefaultGroup.modifyParams('add', {'WALLABY_FORCE_RESTART'=>val}, {})
            else
              ...
              nlist.each do |n|
                ...
                gobj.modifyParams('add', {'WALLABY_FORCE_RESTART'=>val}, {})
              end
